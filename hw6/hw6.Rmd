...
#Mammals_Milk <- data_frame(Mammals_Milk, km_3 = MMilk_km_3$cluster, km_4 = MMilk_km_4$cluster)

Mammals_Milk %>% arrange(km_3)
Mammals_Milk %>% arrange(km_4)

# Code breaking. You need to define variables before you use them. (-2)
```
...
```{r}
MMilk_Agg_K3 <- Mammals_Milk %>% group_by(km_3) %>% summarize(Count=n(), Mean_Protein = mean(Protein), Mean_Water = mean(Water), Mean_Fat = mean(Fat), Mean_Lactose = mean(Lactose), Mean_Ash = mean(Ash))
MMilk_Agg_K3 <- round(MMilk_Agg_K3, 2)
MMilk_Agg_K4 <- Mammals_Milk %>% group_by(km_3) %>% summarize(Count=n(), Mean_Protein = mean(Protein), Mean_Water = mean(Water), Mean_Fat = mean(Fat), Mean_Lactose = mean(Lactose), Mean_Ash = mean(Ash))
...
require(cowplot)
graph1 <-ggplot(Mammals_Milk, aes(x = km_3, y = Protein, group = km_3)) + geom_boxplot() + labs(title = "Protein Distribution")
graph2 <- ggplot(Mammals_Milk, aes(x = km_3, y = Water, group = km_3)) + geom_boxplot() + labs(title = "Water Distribution")
graph3 <- ggplot(Mammals_Milk, aes(x = km_3, y = Fat, group = km_3)) + geom_boxplot() + labs(title = "Fat Distribution")
graph4 <- ggplot(Mammals_Milk, aes(x = km_3, y = Lactose, group = km_3)) + geom_boxplot() + labs(title = "Lactose Distribution")
graph5 <- ggplot(Mammals_Milk, aes(x = km_3, y = Ash, group = km_3)) + geom_boxplot() + labs(title = "Ash Distribution")
...
wgss
plot(c(1:8), wgss, xlab = "Number of Groups")
