...
<<<<<<< Updated upstream
d(1,2) = sqrt((x11+x21)^2+(x12+x22)^2+(x13+x23)^2+(x14+x24)^2) = 
...
head(Vans93)
select(Vans93, Model, Price, EngineSize, Horsepower, MPG.highway, Length)
...
print(Vans_Euclidean)
#The two vans with the shortest distance are Aerostar and Caravan with a distance of 3.44. On the other hand, the largest distance is between the Lumina APV and Eurovan with a distance of 61.80.
```
...
colnames(vertebrate_euclidean) <- rownames(vertebrate)
rownames(vertebrate_euclidean) <- rownames(vertebrate)
euclidean_dendrogram <- hclust(vertebrate_euclidean, method = "complete")
plot(euclidean_dendrogram, main = "Euclidean Vertebrates")
```
...
rownames(vertebrate_euclidean) <- rownames(vertebrate)
euclidean_dendrogram <- hclust(vertebrate_euclidean, method = "complete")
plot(euclidean_dendrogram, main = "Clustered Vertebrates, Euclidean Metric, Complete Linkage", sub = "", xlab = "Vertebrates", cex.lab = 1.25, hang = -1, lwd =3)
```

(ii)using the Manhattan metric with single linkage and
```{r}
vertebrate_manhattan <- as.matrix(dist(vertebrate[, -1], method = "manhattan"))
colnames(vertebrate_manhattan) <- rownames(vertebrate)
rownames(vertebrate_manhattan) <- rownames(vertebrate)
manhattan_dendrogram <- hclust(vertebrate_manhattan, method = "single")
plot(euclidean_dendrogram, main = "Clustered Vertebrates, Euclidean Metric, Complete Linkage", sub = "", xlab = "Vertebrates", cex.lab = 1.25, hang = -1, lwd =3)
```
